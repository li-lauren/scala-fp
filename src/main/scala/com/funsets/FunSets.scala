...
trait FunSets extends FunSetsInterface {
   ...
   * that satisfies `p`.
   */
  def exists(s: FunSet, p: Int => Boolean): Boolean = !forall(s, complement(p))
  def complement(s: FunSet): FunSet = (x: Int) => !contains(s, x)

  /**
   * Returns a set transformed by applying `f` to each element of `s`.
   */
  def map(s: FunSet, f: Int => Int): FunSet = (x: Int) => exists(s, y => f(y) == x)
  // there exists some bounded integer in S -- y -- where f(y) == x; then x is in the transformed set
