...
import com.objsets.TweetReader._
...
abstract class TweetSet extends TweetSetInterface {
  ...
  def keywordTweets(keywords: List[String]): TweetSet
...
class Empty extends TweetSet{
  ...
  def mostRetweeted: Tweet = new Tweet("blah", "blah", -1)

  def keywordTweets(keywords: List[String]): TweetSet = this
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    if (L.retweets > elem.retweets && L.retweets > R.retweets) L
    else if (R.retweets > elem.retweets && R.retweets > L.retweets) R
    else elem
  }

  def keywordTweets(keywords: List[String]): TweetSet = {
    def hasKeyword(t: Tweet): Boolean = keywords.exists(keyword => t.text.contains(keyword))
    if (hasKeyword(elem)) left.keywordTweets(keywords) union right.keywordTweets(keywords) incl elem
    else left keywordTweets(keywords) union right.keywordTweets(keywords)
  }
...
//  newTestSet foreach println
  //val keywords = List("blah")
  //println(newTestSet.keywordTweets(keywords) foreach println)
  //println(newTestSet.descendingByRetweet foreach println)
